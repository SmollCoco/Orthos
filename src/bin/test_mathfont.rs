...
fn main() {
    ...
    let all_fonts: Vec<Font> = font_data
        .iter()
        .filter_map(|(data, index)| Font::new(Bytes::new(*data), *index))
        .collect();

    if all_fonts.len() != 7 {
        eprintln!("Warning: expected 7 fonts, loaded {}", all_fonts.len());
    }
    ...
    match result.output {
        Ok(doc) => {
            ...
            std::fs::write("/tmp/out_mathfont.pdf", &pdf).unwrap();
            println!("OK: wrote /tmp/out_mathfont.pdf ({} bytes)", pdf.len());
        }
        Err(diags) => {
            for d in &diags {
